...
async function generatePackageJson(config) {
  const packageData = {
    ...
    main: config.typescript ? 'dist/main/main.js' : 'src/main/main.js',
    homepage: './',
    author: '',
    license: 'MIT',
...
function getScripts(config) {
  ...
  if (config.typescript) {
    ...
    scripts['dev:renderer'] =
      'webpack serve --config webpack.renderer.config.js --mode development';
    ...
    scripts['dev'] = 'electron . --inspect=5858';
  }
